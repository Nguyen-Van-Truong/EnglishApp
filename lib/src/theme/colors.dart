// lib/src/theme/colors.dart
import 'package:flutter/material.dart';
import 'package:color_type_converter/color_type_converter.dart';
import 'package:hexcolor/hexcolor.dart';

class AppColors {
  static const Map<int, Map<String, Color>> themes = {
    0: {
      'navigationBarBackground': Color(0xFF13112F),
      'chooseNavigationBarBackground': Color(0xFF000000),
      'pageBackground': Color(0xFF38036D),
      'cardBackground': Color(0xFF2D0259),
      'cardBackgroundOrange': Color(0xFFFFA500),
      'searchDictionaryBackground': Color(0xFFFF6A00),
      'cardChatBot': Color(0xFF38036D),
      'border': Color(0xFFD5D5D5),
      'continueSubscriptionBackground': Color(0xFFF5B000),
      'buttonContinueSubscription': Color(0xFFFFB700),
      'seeAllVocabularyList': Color(0xFFFF6A00),
      'messageUserBackground': Color(0xFF13112F),
      'studyThisSet': Color(0xFFFF8000),
      'messageBotBackground': Color(0xFF1E0101),
      'headerBackgroundGradient2Home': Color(0xFFEDDBDB),
      'headerBackgroundGradient1Home': Color(0xFFF5B000),
      'headerBackground': Color(0xFFFFA6DB),
      'buttonCall': Color(0xFFFF0000),
      'headerBackgroundProfile': Color(0xFFFF8200),
      'headerCircle1': Color(0xFFE1C9FF),
      'headerCircle2': Color(0xFFB57DFE),
      'headerCircle3': Color(0xFFD9D9D9),
      'headerCircle4': Color(0xFFFFE082),
      'primaryText': Colors.white,
      'primaryTextBlack': Colors.black,
      'primaryTextHeader': Colors.white,
      'secondaryText': Colors.white70,
      'chatBubbleBackground': Colors.blue,
      'footerBackground': Colors.blue,
      'footerVocabularyNameBackground': Color(0xFFFFE082),

    },
    1: {
      'navigationBarBackground': Color(0xFFFF39C7),
      'chooseNavigationBarBackground': Color(0xFF000000),
      'pageBackground': Color(0xFFFFC0CB),
      'cardBackground': Color(0xFFFFB6C1),
      'cardBackgroundOrange': Color(0xFFFFA500),
      'searchDictionaryBackground': Color(0xFFFF6A00),
      'cardChatBot': Color(0xFFFFFFFF),
      'border': Color(0xFFD5D5D5),
      'continueSubscriptionBackground': Color(0xFFF5B000),
      'buttonContinueSubscription': Color(0xFFD98A19),
      'seeAllVocabularyList': Color(0xFFFF6A00),
      'messageUserBackground': Color(0xFFFF39C7),
      'studyThisSet': Color(0xFFFF8000),
      'messageBotBackground': Color(0xFFFF39C7),
      'headerBackgroundGradient2Home': Color(0xFFEDDBDB),
      'headerBackgroundGradient1Home': Color(0xFFF5B000),
      'headerBackground': Color(0xFFFFE4E1),
      'buttonCall': Color(0xFFFF0000),
      'headerBackgroundProfile': Color(0xFFFF8200),
      'headerCircle1': Color(0xFFFFDAB9),
      'headerCircle2': Color(0xFFFFA07A),
      'headerCircle3': Color(0xFFF08080),
      'headerCircle4': Color(0xFFFFE082),
      'primaryText': Colors.black,
      'primaryTextHeader': Colors.white,
      'secondaryText': Colors.black87,
      'chatBubbleBackground': Colors.pinkAccent,
      'footerBackground': Colors.pinkAccent,
      'footerVocabularyNameBackground': Color(0xFFFFE082),

    },
    2: {
      'navigationBarBackground': Color(0xFFFFFFFF),
      'chooseNavigationBarBackground': Color(0xFFFFA600),
      'pageBackground': Color(0xFFFFFFFF),
      'pageAndFooterBackgroundCenterChatbot': Color(0xFFFFDEC0),
      'pageAndFooterBackgroundCenterCheckSpelling': Color(0xFFFFDEC0),
      'cardBackground': Color(0xFFFFFFFF),
      'cardBackgroundOrange': Color(0xFFF1BC62),
      'searchDictionaryBackground': Color(0xFFFF8429),
      'cardChatBot': Color(0xFFFFFFFF),
      'border': Color(0xFFD5D5D5),
      'continueSubscriptionBackground': Color(0xFFF5B000),
      'buttonContinueSubscription': Color(0xFFFF6A00),
      'seeAllVocabularyList': Color(0xFFFF6A00),
      'messageUserBackground': Color(0xFFFF9100),
      'iconChatbotPrimary': Color(0xFFFF9100),
      'iconCheckSpellingPrimary': Color(0xFFFF9100),
      'studyThisSet': Color(0xFFFF8000),
      'messageBotBackground': Color(0xFFFFFFFF),
      'headerBackgroundGradient2Home': Color(0xFFEDDBDB),
      'headerBackgroundGradient1Home': Color(0xFFF5B000),
      'headerBackground': Color(0xFFEACE8D),
      'buttonCall': Color(0xFFFF0000),
      'headerBackgroundProfile': Color(0xFFFF9700),
      'headerCircle1': Color(0xFF00FF14),
      'headerCircle2': Color(0xFFF5B000),
      'headerCircle3': Color(0xFFA9A6A6),
      'headerCircle4': Color(0xFFFFE082),
      'primaryText': Colors.black,
      'primaryTextWhite': Colors.white,
      'primaryTextHeader': Colors.white,
      'secondaryText': Colors.black87,
      'chatBubbleBackground': Colors.amber,
      'footerBackground': Colors.amber,
      'footerVocabularyNameBackground': Color(0xFFFFE082),
      'buttonFinish': Color(0xFFFFFB00),
    },
  };

  static Color getColor(int themeIndex, String colorKey) {
    return themes[themeIndex]?[colorKey] ?? Colors.black;
  }
}
